...
import { InfrastructureError, ValidationError, AppError } from '@/shared/utils/application-error';
...
export class NotificationProcessor {
  ...
  async process(job: Job) {
    ...
    console.log(`[Worker] Started processing: ${notificationId} (Job ${job.id})`);

    // 1. Fetch notification
    const notification = await this.notificationRepository.findById(notificationId);
    if (!notification) {
      throw new InfrastructureError(`Notification ${notificationId} not found in database`);
    }
    ...
    try {
      ...
      try {
        validatePayload(notification.type as NotificationType, notification.payload);
      } catch (error) {
      ...
      if (response.success) {
        // 5a. Success Flow
        const finalUpdate = await this.notificationRepository.updateStatus(notificationId, NotificationStatus.SENT, 1);
        ...
        await this.handleFailure(job, notificationId, response.error || 'Provider rejected', response.metadata);
      }
    } catch (err: any) {
      // 6. Infrastructure or Uncaught Failure
      const message = err instanceof AppError ? err.message : `System Failure: ${err.message}`;
      await this.handleFailure(job, notificationId, message, { originalError: err });
   ...
   */
  private async handleFailure(job: Job, notificationId: string, error: string, metadata?: any) {
    ...
    if (isLastAttempt) {
      ...
      console.warn(`[Worker] Retry Failure: ${notificationId} | attempt ${job.attemptsMade + 1} | ${error}`);
    }
  }
}
